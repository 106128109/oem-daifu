...
class OrderController extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->assign("banklist", $banklist);
         //子账号
        $zizhanghaolist = M("channel_account")->field('id,title,mch_id')->select();
        $this->assign("zizhanghaolist", $zizhanghaolist);
        
        $where    = array();
        $memberid = I("request.memberid");
        if ($memberid) {
            $where['O.pay_memberid|pay_username'] = array('eq', $memberid);
        ...
        if ($body) {
            $where['O.pay_username'] = array('eq', $username);
        }
        ...
        if ($body) {
            $where['O.pay_productname|pay_amount|true_amount'] = array('eq', $body);
        }
        $this->assign('body', $body);
         $mobile = I("request.mobile", '', 'strip_tags');
        if ($mobile) {
            $where['O.mobile'] = array('eq', $mobile);
        }
        $this->assign('mobile', $mobile);
         $order_ip = I("request.order_ip", '', 'strip_tags');
        if ($order_ip) {
            $where['O.order_ip|yichangip'] = array('eq', $order_ip);
        }
        $this->assign('order_ip', $order_ip);
         $zizhanghao = I("request.zizhanghao", '', 'strip_tags');
        if ($zizhanghao) {
            $where['O.account_id'] = array('eq', $zizhanghao);
        }
        $this->assign('zizhanghao', $zizhanghao);
        ...
        $createtime1 = urldecode(I("request.createtime"));
       $createtime=htmlentities($createtime1); //20190413更新过滤特殊字符 反射型xss 中危
        if ($createtime) {
        ...
        $successtime1 = urldecode(I("request.successtime"));
        $successtime=htmlentities($successtime1); //20190413更新过滤特殊字符 反射型xss 中危
        if ($successtime) {
        ...
        $field                    = ['sum(`pay_amount`) pay_amount','sum(`true_amount`) true_amount','sum(`cost`) cost', 'sum(`pay_poundage`) pay_poundage', 'sum(`pay_actualamount`) pay_actualamount', 'count(`id`) success_count'];
        ...
        //投诉保证金冻结金额
        $zongdindgan = $sum['fail_count'] +  $sum['success_count'];
        $chenggonglv = round($sum['success_count'] / $zongdindgan * 100,2);
        /*时间*/
              $dqshijian=date("Y-m-d H:i:s");
              $sanshifenzhong=date("Y-m-d H:i:s",strtotime("-30 minutes"));
              $yixiaoshi=date("Y-m-d H:i:s",strtotime("-1 hours"));
              $sanxiaoshi=date("Y-m-d H:i:s",strtotime("-3 hours"));
              $liuxiaoshi=date("Y-m-d H:i:s",strtotime("-6 hours"));
              $zuotian=date("Y-m-d H:i:s",strtotime("-1 day"));
              $str11=date("Y-m-d",strtotime("0 day"))." 0:0:0";
		      $str222=date("Y-m-d",strtotime("0 day"))." 23:59:59";
              $zrone=date("Y-m-d",strtotime("-1 day"))." 0:0:0";
		      $zuoritwo=date("Y-m-d",strtotime("-1 day"))." 23:59:59";
              /*时间end*/ 
              /*计算成功率*/
           
       $banxiaoshi =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between', [strtotime($sanshifenzhong), strtotime($dqshijian)]], 'pay_status'=>['in',[1,2]]])->count();
       $banxiaoshisum =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between', [strtotime($sanshifenzhong), strtotime($dqshijian)]]])->count();
        $banxiaoshicgl = sprintf("%.2f",$banxiaoshi / $banxiaoshisum * 100);//半小时成功率
       $onetime =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between', [strtotime($yixiaoshi), strtotime($dqshijian)]], 'pay_status'=>['in',[1,2]]])->count();
       $onetimesum =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between', [strtotime($yixiaoshi), strtotime($dqshijian)]]])->count();
        $onetimecgl = sprintf("%.2f",$onetime / $onetimesum * 100);//1小时成功率
       $sanxiaoshi1 =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between', [strtotime($sanxiaoshi), strtotime($dqshijian)]], 'pay_status'=>['in',[1,2]]])->count();
       $sanxiaoshisum =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between', [strtotime($sanxiaoshi), strtotime($dqshijian)]]])->count();
        $shours = sprintf("%.2f",$sanxiaoshi1 / $sanxiaoshisum * 100);//3小时成功率
       $liuxiaoshi1 =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between', [strtotime($liuxiaoshi), strtotime($dqshijian)]], 'pay_status'=>['in',[1,2]]])->count();
       $liuxiaoshisum =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between', [strtotime($liuxiaoshi), strtotime($dqshijian)]]])->count();
        $liutimecgl = sprintf("%.2f",$liuxiaoshi1 / $liuxiaoshisum * 100);//6小时成功率
       $dangri =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between', [strtotime($str11), strtotime($str222)]], 'pay_status'=>['in',[1,2]]])->count();
       $dangrisum =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between',  [strtotime($str11), strtotime($str222)]]])->count();
        $dangricgl = sprintf("%.2f",$dangri / $dangrisum * 100);//当日成功率
       $zrtime =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between',  [strtotime($zrone), strtotime($zuoritwo)]], 'pay_status'=>['in',[1,2]]])->count();
       $zrtimesum =  M('Order')->alias('as O')->where($where)->where(['pay_applydate'=>['between',  [strtotime($zrone), strtotime($zuoritwo)]]])->count();
        $zrcgl = sprintf("%.2f",$zrtime / $zrtimesum * 100);//昨日成功率
       //var_dump($yonghu,$banxiaoshi);
              /*计算成功率end*/
      
        //var_dump($chenggonglv);
        $map = $where;
        $map['C.status'] = 0;
        ...
        if ($status == '1or2' || $status == 1 || $status == 2) {
            ...
            $stat['todaytruesum'] = M('Order')->where($todaysumMap)->sum('true_amount');
        ...
        $this->assign('t_amount',$sum['true_amount']);
        ...
        $this->assign('chenggonglv', $chenggonglv); //总成功率
        $this->assign('banxiaoshicgl', $banxiaoshicgl);//半小时
        $this->assign('onetimecgl', $onetimecgl);//一小时
        $this->assign('shours', $shours);//3小时
        $this->assign('liutimecgl', $liutimecgl);//6小时
        $this->assign('dangricgl', $dangricgl);//当日
        $this->assign('zrcgl', $zrcgl);//昨日
  ...
  /**
    ...
    public function wsryshow()
    {
  	...
  	public function doForzenWsry(){
		...
		if($id)
        {
        	$res = M('wsry')->where(array('id'=>$id))->save(array('status'=>$status));
          	if($res)
            {
            	$this->success('修改成功');
            }
  ...
  //虚拟订单结束
    ...
    public function dfApiOrderList()
    {
        ...
        $this->assign('status', $status);
        $create_time1 = urldecode(I("request.create_time"));
       $create_time=htmlentities($create_time1); //20190413更新过滤特殊字符 反射型xss 中危
        if ($create_time) {
        ...
        $check_time1 = urldecode(I("request.check_time"));
       $check_time=htmlentities($check_time1); //20190413更新过滤特殊字符 反射型xss 中危
        if ($check_time) {
    ...
    public function delAll()
   {
    ...
    public function doForzen(){
     ...
     public function fengIp(){
     	$orderId= I('orderid/s',0);
     //  $orderId = '20201108103241575053';
        if(!$orderId)
            $this->error("订单ID有误");
        $order=M("order")->where(['pay_orderid'=>$orderId])->find();
        
       // pay_channel
        $tongdao111=M("channel")->where(['id'=>$order['channel_id']])->find();
        if(strstr($tongdao111['iphei'],$_POST['ip'])){
        	 $this->success('该IP之前已封禁，不可重复操作');
        	
        }else{
	     	$ip = $tongdao111['iphei'].",".$_POST['ip'];
	     	
	        $ret=M("channel")->where(['id'=>$order['channel_id']])->save(array('iphei'=>trim($ip,',')));
	
	         $this->success('IP:'.$_POST['ip'].'封禁成功！');
    ...
    public function frozenOrder(){
        ...
        $body = I("request.body", '', 'strip_tags');
        if ($body) {
            $where['O.pay_productname|pay_amount'] = array('eq', $body);
        }
        $this->assign('body', $body);
         $mobile = I("request.mobile", '', 'strip_tags');
        if ($mobile) {
