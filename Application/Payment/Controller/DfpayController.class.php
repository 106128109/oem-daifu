...
class DfpayController extends Controller
{
    ...
    public function add()
    {
        ...
        if(!$sign) {
            $this->showmessage("缺少签名参数或未设置默认代付通道");
        }
...
//        if(!$this->merchants['df_api']) {
//            $return = ['status' => 0, 'msg' => '商户未开启此功能'];
//        }
        if($this->merchants['df_domain'] != '') {
            $referer = getHttpReferer();
            if(!checkDfDomain($referer, $this->merchants['df_domain'])) {
                $return = ['status' => 0, 'msg' => '请求来源域名与报备域名不一致'];
            }
        }
        if($this->merchants['df_ip'] != '' && !checkDfIp($this->merchants['df_ip'])) {
            $return = ['status' => 0, 'msg' => 'IP地址与报备IP不一致'];
        }
        $request = [
            'mchid'=>$mchid,
            'timestamp'=>$timestamp
        ...
        $signature = $this->createSign($this->merchants['apikey'],$request);
        if($signature != $sign){
            $return = ['status' => 0, 'msg' => '验签失败'];
        }
        $data = array();
        if(!$return){
            $data['mchid']          = $mchid;
            $data['balance']        = $this->merchants['balance'];
            $data['blockedbalance'] = $this->merchants['blockedbalance'];
            $data['sign'] = $this->createSign($this->merchants['apikey'],$data);
            $return = ['status' => 1, 'msg' => '余额查询成功'];
        }
        $return['time'] = time();
        $return['data'] = $data;
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode($return,320);
        exit;
    }
