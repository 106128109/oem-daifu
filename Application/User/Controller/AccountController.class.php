...
//查询是否开启短信验证
    ...
    {
        $res = $this->send('addBankcardSend', $this->fans['mobile'], '更新银行卡');
        $this->ajaxReturn(['status' => $res['code']]);
    }
       ...
       $userid = session('user_auth.uid');
        ...
        //查询是否开启短信验证
            ...
            if (!$p['oldpwd'] || !$p['newpwd'] || !$p['secondpwd'] || $p['newpwd'] != $p['secondpwd'] ||
                ...
                $this->ajaxReturn(['status' => 0, 'msg' => '原始密码输入错误']);
            }
...
$site = ((is_https()) ? 'https' : 'http') . '://' . $this->siteconfig['domain'];
...
// 上传成功
    ...
    {
        $res = $this->send('editPasswordSend', $this->fans['mobile'], '登录密码');
        $this->ajaxReturn(['status' => $res['code']]);
    }
    /**
     * 发送修改支付密码的验证码信息
        ...
        if (!IS_POST) {
            ...
            if($google_token == '') {
              ...
              $http_type = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') || (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] == 'https')) ? 'https://' : 'http://'; 
                $qrCodeUrl = $ga->getQRCodeGoogleUrl($http_type . $_SERVER['HTTP_HOST'] .'@'.$this->fans['username'], $secret);
    ...
    public function complaintsDeposit()
    {
        ...
        $createtime1 = urldecode(I("request.createtime"));
       $createtime=htmlentities($createtime1); //20190323更新过滤特殊字符 反射型xss 中危
        if ($createtime) {
            list($cstime, $cetime) = explode('|', $createtime);
    ...
    public function reconciliation()
    {
       // if($this->fans[groupid] != 4) {
        ...
        //}
        $date1 = urldecode(I("request.date", ''));
		$date=htmlentities($date1); //20190413更新过滤特殊字符 反射型xss 中危
        if(!$date) {//默认今日
            $date = date('Y-m-d');
        }
    ...
    private function getDayReconciliation($begin, $end) {
        ...
        if(empty($data) || (!empty($data) && diffBetweenTwoDays(date('Y-m-d'),$date)<=30)) {//30天内账单实时更新数据
            ...
            //订单总额
            $data['order_total_amount'] = M('Order')->where(['pay_memberid'=>$this->fans['memberid'],'pay_applydate' => ['between', [strtotime($begin), strtotime($end)]]])->sum('pay_actualamount');
            //订单实付总额
			...
			//订单未付总额
			$data['order_success0_amount'] = M('Order')->where(['pay_memberid'=>$this->fans['memberid'],'pay_applydate' => ['between', [strtotime($begin), strtotime($end)]], 'pay_status' =>0])->sum('pay_actualamount');
           //费率总额
            $data['order_poundage_amount'] = M('Order')->where(['pay_memberid'=>$this->fans['memberid'],'pay_successdate' => ['between', [strtotime($begin), strtotime($end)], 'pay_status' => ['in', '1,2']]])->sum('pay_poundage');
            if($insertFlag) {
